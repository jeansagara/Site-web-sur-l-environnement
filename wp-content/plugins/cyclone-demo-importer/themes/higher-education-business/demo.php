...
function cdi_import_files() {
    return array(
        array(
            'import_file_name'             => __('Higher Education Business Free','cdi'),
            'categories'                   =>  array( 'Free' , 'Homepage' ),
            'local_import_file'            => CDI_PLUGIN_DIR_PATH . '/themes/higher-education-business/inc/content.xml',
            'local_import_customizer_file' => CDI_PLUGIN_DIR_PATH . '/themes/higher-education-business/inc/options.dat',
            'import_preview_image_url'     => CDI_PLUGIN_DIR_URL . '/assets/images/education-business/homepage-higher-education.jpg',
            'preview_url'                  => 'https://bizbergthemes.com/higher-education-business/'
        ),
        array(
...
function cdi_after_import_setup( $selected_import ) {
    ...
    switch ( $import_file_name ) {
        ...
        case 'Higher Education Business Free':
             $front_page_id = get_page_by_title( 'Higher Education Business Homepage' );
            break;
    ...
    set_theme_mod( 
        'transparent_header_pages', 
        json_encode( array(
            array( 'page_id' => $front_page_id->ID ),
        ) )
    );
